...
## تسک برنامه‌ریزی: از یک جمله‌ی مدیریتی تا batch dispatch

این بخش مشخصات تغییری است که باید در n8n اعمال شود تا یک تسک Notion بتواند
زنجیره‌ی برنامه‌ریزی ناوگان را راه بیندازد. تصمیم گرفته شد که **فقط تسک‌های
علامت‌خورده** این کار را بکنند، نه هر تسکی.

### دو مارکر متفاوت، دو مسیر متفاوت

ناوگان دو ورودی جدا دارد و اشتباه گرفتنشان یعنی هیچ‌کدام کار نکند:

| مارکر انتهای متن | چه چیزی را راه می‌اندازد | نتیجه |
|---|---|---|
| `<tag> @agy` | `agy-issue-bot.yml` | **یک** تسک به **یک** worker |
| `<tag> @agy-plan` | `agy-plan-bot.yml` | برنامه‌ریزی کامل ناوگان |

`<tag>` یکی از `ccp`، `cr`، `coffeenet`، `status` است.

### زنجیره‌ی برنامه‌ریزی

```
issue که متنش به «<tag> @agy-plan» ختم می‌شود
   v
agy-plan-bot  ->  وضعیت پروژه را می‌خواند، یک پیشنهاد JSON کامنت می‌کند:
                  {"kind":"tbs_account_plan","packages":[
                     {"account":<0-8، هرگز 6 یا 9>,"package_goal":"..."}]}
   v
[دروازه‌ی انسانی]  کامنتی که به «@agy-plan-approved» ختم شود
   v
agy-plan-dispatch  ->  برای هر حساب، agy-lead-plan را dispatch می‌کند
   v
agy-lead-plan  ->  lead هر حساب بسته را بین ۱۰ زیردست می‌شکند
   v
agw-worker  ->  اجرای هر تسک
   v
agy-final-report  ->  آخرین حسابی که تمام کند، جمع‌بندی می‌کند
```

**تا قبل از `@agy-plan-approved` هیچ کدی اجرا نمی‌شود.** فقط یک پیشنهاد تولید
می‌شود. به همین دلیل وصل کردن Notion به این مسیر کم‌ریسک است.

اگر پیشنهاد را نپسندیدید، دوباره با `@agy-plan` کامنت بگذارید — بات کل گفتگو را
می‌خواند و یک پیشنهاد کامل جدید می‌دهد، نه وصله‌ی پیشنهاد قبلی.

### آنچه در Notion لازم است

یک property جدید از نوع **checkbox** به Task Board اضافه شود، مثلاً `Plan`.

تسکی که این تیک را دارد، درخواست برنامه‌ریزی است. بقیه تسک عادی می‌مانند.

مقادیر property موجودِ `Project` باید به تگ‌های بالا نگاشت شوند. اگر مقادیر
Notion اسم کامل پروژه‌اند (مثلاً `Claud-Cloud-Project`)، نگاشت لازم است:
...
### آنچه در n8n لازم است

در ورک‌فلوی `Notion Event → agw-workers Issue (Webhook)`، قالب body شرطی شود.

**تسک عادی** (بدون تیک `Plan`) — بدون تغییر نسبت به امروز:
...
### سه قید که اگر رعایت نشوند هیچ‌چیز کار نمی‌کند

**۱. ترتیب.** `agy-plan-bot` متن را با این تست می‌سنجد:
...
یعنی `<tag> @agy-plan` باید **آخرین چیز** باشد. مارکر `notion-page-id` باید
**قبل** از آن بیاید. اگر مارکر آخر باشد، شرط هرگز برقرار نمی‌شود و تسک بی‌صدا
نادیده گرفته می‌شود — بدون هیچ خطایی، در هیچ لاگی.

**۲. ریپوی مقصد.** `agy-plan-bot.yml` فقط روی `mohammadlali0707-stack/agw-workers`
وجود دارد. issue باید آنجا ساخته شود، نه روی حساب دیگری.

**۳. هویت سازنده.** شرط اجرا این است:

```
author_association == 'OWNER' || 'COLLABORATOR' || user.login == 'Mohammadlali'
```

issue که با `ACC6_PAT` روی ریپوی خودِ ACC6 ساخته شود `OWNER` می‌شود، پس این قید
خودبه‌خود برقرار است.

### نکته درباره‌ی `Status`

تسک برنامه‌ریزی هم مثل بقیه به `In Progress` می‌رود و با بسته شدن issue به `Done`
برمی‌گردد. ولی «بسته شدن issue برنامه‌ریزی» یعنی پیشنهاد تمام شد، نه اینکه کار
انجام شد — کارِ واقعی در ریپوی پروژه انجام می‌شود و issueهای خودش را دارد.

اگر این تمایز مهم است، یک مقدار جدا در `Status` برایش در نظر بگیرید.
